# HANDOVER TO CLAUDE-D — problems-extend authoring, PARTITION D (resumable, 2026-08-23)

You (Claude-D) own ONE PARTITION of the problems-extend authoring wave,
running in PARALLEL with three other sessions: Partition A (the original
Claude coordinator), Partition C (Claude-C), and Partition B (Codex).
This memo is resumable: a fresh Claude-D session should select and claim
its next shard from its canonical tracker without asking the user. Read
CLAUDE.md in both repos first; this memo covers the partition split,
startup selection, conflict protocol, and the authoring loop.

## PARTITION ASSIGNMENT

Partition D was carved on 2026-08-23 from the upper range of B at the
shard boundary **id 3001**. Shard dirs are disjoint, so the four sides
never touch each other's files:

| Side | Shards | Entries | Ownership |
|---|---|---|---|
| **A (Claude, original)** | 0001-0100 … 1401-1500 + merge duties | — | owns shared trackers; active in the low shards |
| **C (Claude-C)** | default-shared low range (see its events.json) | — | sole owner wherever it has claimed |
| **B (Codex)** | 2001-2100 … 2901-3000 | 827 pending | sole owner of ids 2001..3000 (active ~2100s) |
| **D (Claude-D, you)** | 3001-3100 … 4001-4100 | 910 pending | sole owner from 3001 onward |

**Your scope**: author every `pending` problem in shards 3001-3100
through 4001-4100, in tracker order, shard by shard. NEVER author inside
any shard below 3001 — Codex owns 2001..3000 and is actively working
upward toward it, and the low shards belong to A/C.

Note id 3000 itself lives in shard 2901-3000 and stays Codex's; your
first id is 3001. The last problem in the corpus is 4018
(total-sum-of-interaction-cost-in-tree-groups-ii) in shard 4001-4100.

The tracker is intentionally SPARSE. An id absent from your
`.localonly/claude-d/partition.json` is out of scope (usually already in
`problems-bettercode`). Never infer the next problem by incrementing an
id, and never author an absent id.

## STARTUP — PICK AND CLAIM THE NEXT SHARD YOURSELF

A fresh Claude-D session does not need a user-selected shard:

1. Parse `openoj-problems/.localonly/claude-d/partition.json` and verify
...
Partition D's bookkeeping has one writer at a time. Do not run two
independent Claude-D sessions against `.localonly/claude-d/`
concurrently; a second concurrent fleet needs a separately seeded
disjoint tracker and bookkeeping directory.

## CONFLICT PROTOCOL (what makes four-way parallelism safe)
...
- `.localonly/claude-c/` — Claude-C's bookkeeping (read-only for you).
- `.localonly/claude-d/` is YOURS alone.

Your own bookkeeping (seeded 2026-08-23, gitignored):

- `partition.json` — your tracker: all 910 entries with id/slug/status.
...
Scratch scripts: prefix yours `claude-d_` in `.localonly/` to avoid
...
1. Maintain as many concurrent authoring subagents as the session
   exposes, up to the user's cap for this session (recent fleets ran
   4-8 total slots including the coordinator; confirm once if unset).
   Refill immediately on every completion.
...
5. On subagent death by 429: resume instantly (context survives via
   ...
   NOTE: FOUR fleets now SHARE one account's rate limits — deaths may
   come from any other side's consumption. Still resume instantly.
...
- Mirror exemplar `problems-bettercode/0001-0100/0001_two-sum/` +
  ...
  (string construction). Never read in-flight bundles (other sides are
  active below 3001; anything not yet `done` in their trackers may be in
  flight).
...
- Two string-array and two-matrix params both work end-to-end (0539,
  ...
  proven.
- Rust runtime panics print doubled braces in the runtime_error JSON and
  can misreport as "unparseable protocol output" — when a Rust run fails
  that way, suspect a panic, not the wire format.

## STATE AT HANDOVER (your side)

- Nothing in shards 3001-4100 is authored; your partition.json has all
  910 entries `pending`; no bundle directories exist on disk.
- Shard sizes (pending): 3001-3100: 84, 3101-3200: 84, 3201-3300: 89,
  3301-3400: 89, 3401-3500: 74, 3501-3600: 87, 3601-3700: 96,
  3701-3800: 95, 3801-3900: 97, 3901-4000: 98, 4001-4100: 17.
- Codex is concurrently landing batches from 2001 upward and will
  approach 2901-3000 eventually; stay strictly at 3001+.
